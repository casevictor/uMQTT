...
extension OutputStream {
    ...
    func writeData(_ data: [UInt8]) -> Int {
        return data.withUnsafeBytes { rawBufferPointer in
            guard let base = rawBufferPointer.bindMemory(to: UInt8.self).baseAddress else { return -1 }
            let size = data.count
            var completed = 0
            while completed < size {
                let wrote = write(base.advanced(by: completed), maxLength: size - completed)
                if wrote <= 0 {
                    return wrote
                } else {
                    completed += wrote
...
protocol uMQTTDelegate: AnyObject {
...
class uMQTT : NSObject, StreamDelegate, @unchecked Sendable {
    ...
    init(host: String, atPort:UInt32 = 1883, autoconnect: Bool = true){
        self.host = host
        self.port = atPort
...
#else
        fatalError("Network streams are not supported on this platform")
#endif
    }
    ...
    func openMQTTSocket() -> () {
        self.connectSocket(self.host, port: self.port)
    ...
    func ping() -> () {
        ...
        let bytesToWire = frame.buildFrame()
        bytesToWire.withUnsafeBytes { ptr in
            guard let base = ptr.bindMemory(to: UInt8.self).baseAddress else { return }
            self.outputStream!.write(base, maxLength: bytesToWire.count)
        }
    }
    ...
    fileprivate func startKeepAliveTimer() {
        ...
        keepAliveTimer = Timer.scheduledTimer(withTimeInterval: Double(keepAliveInterval) / 2.0, repeats: true) { [weak self] _ in
            self?.ping()
        }
    }
...
private class uMQTTConnectFrame : uMQTTFrame {
    ...
    override func buildPayload() -> () {
        let clientId = userDefaults.string(forKey: "mqttClientId") ?? "uMQTT"
